...
with base as (
select *
from {{ ref('int_yt_metric_definition') }}
),

country_dedupe as (

select *, row_number() over (partition by video_id, trending_date order by view_count desc nulls last) as country_dedupe
from base
),
...
select * exclude(country_dedupe), 
        row_number() over (partition by video_id order by trending_date asc nulls last) as first_trending_date_dedupe
from country_dedupe
where country_dedupe = 1
)


select * exclude(first_trending_date_dedupe),
